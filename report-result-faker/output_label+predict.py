...
COMBINED_OUT = SEG_OUT_DIR / "labels_vs_predictions_square_2x2.png"
...
def main():
    ...
    for seg_path in SEG_JSONS:
        ...
        # LEFT: geometry only (seg + boxes)
        left_img = _draw_seg_geometry(img, segments)
        left_img = _draw_boxes_geometry(left_img, boxes)
        left_tiles.append(left_img)

        # RIGHT: geometry + text for seg and boxes (tree labels included here)
        right_img = _draw_predictions(img, segments, boxes)
        right_tiles.append(right_img)

    if not left_tiles or not right_tiles:
        print("[ERROR] nothing to render.")
        return

    left_grid  = _grid_2x2_square(left_tiles,  TILE)
    right_grid = _grid_2x2_square(right_tiles, TILE)
    canvas = _compose_canvas(left_grid, right_grid)

    COMBINED_OUT.parent.mkdir(parents=True, exist_ok=True)
    cv2.imwrite(str(COMBINED_OUT), canvas)
    print(f"[OK] Saved {COMBINED_OUT}")
